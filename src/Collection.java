...
public class Collection extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		PrintWriter out = response.getWriter();
		if("0".equals(cindex)){
			String sql = "select * from PKM_Collection where userid=" + "'" + uid + "'";
			JSONArray json=new PJsonArray().getJSON(sql,"collection");
			out.print(json);
		}
		if("1".equals(cindex)){
			...
			String sql = "insert into PKM_Collection (userid,collectionaddress,collectionname,collectiontime) values(?,?,?,?)";
			try{
